...
const showExpiredMsg = ref(false)

onMounted(() => {
  if (window.location.search.includes('expired=true')) {
    showExpiredMsg.value = true
...
<template>
  <AuthLayout title="Welcome Back" subtitle="Sign in to your Eventilize account">
    <div v-if="showExpiredMsg" class="alert alert-warning">Your session has expired. Please sign in again.</div>
